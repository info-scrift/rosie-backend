 ...
 * /api/applicant/profile:
 *   post:
 *     summary: Create applicant profile (no resume)
 *     tags: [Applicant]
 *     security: [ { bearerAuth: [] } ]
 *     requestBody:
 *       required: true
 ...
 *               phone: { type: string }
 *               address: { type: string }
 *               Professional_Bio: { type: string, nullable: true }
 *               Curent_Job_Title: { type: string, nullable: true }
 *               Portfolio_link: { type: string, nullable: true }
 ...
 */
export const createApplicantProfileJson = async (req: Request, res: Response) => {
    const userId = req.user?.id;
    const emailFromJwt = req.user?.email ?? null;
    if (!userId) return res.status(401).json({ message: "Unauthorized" });
    ...
    const {
        ...
        skills,
        experience_years,
        email, // if client sends, we’ll prefer JWT email instead
    } = req.body ?? {};
    ...
    try {
        ...
        const normalizedSkills = Array.isArray(skills)
            ? skills.filter((s: any) => typeof s === "string").map((s: string) => s.trim()).filter(Boolean)
            : [];

        const insertRow = {
            id: userId,                     // <<<<<< key change
            email: emailFromJwt ?? email ?? null,
            first_name: first_name ?? null,
            last_name: last_name ?? null,
            phone: phone ?? null,
            address: address ?? null,
            Professional_Bio: Professional_Bio ?? null,
            Curent_Job_Title: Curent_Job_Title ?? null,
        ...
        console.error("createApplicantProfileJson error:", e);
...
export const updateApplicantProfileHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        // IMPORTANT: applicant_profiles uses `id` (== auth.uid())
        const { data: existing, error: selErr } = await supabase
            .from("applicant_profiles")
            .select("id")
            .eq("id", userId)             // <-- correct column
            .maybeSingle();

        if (selErr) {
            console.error("update select error:", selErr.message);
            return res.status(500).json({ message: selErr.message });
        }

        if (!existing) {
            // If RLS blocks the row, you'll also hit this path (no error, no row).
            console.warn("update: no visible row for user:", userId);
            return res.status(404).json({ message: "Applicant profile not found. Create it first." });
        }
        ...
        if (updErr) {
            ...
            return res.status(400).json({ message: updErr.message });
        }

        return res.status(200).json({ message: "Profile updated successfully", profile: updated });
    } catch (e: any) {
        console.error("updateApplicantProfileHandler error:", e);
        return res.status(500).json({ message: e?.message || "Server error" });
    }
};
...
// POST /api/applicant/profile/ensure
// routes: POST /api/applicant/profile/ensure
export const ensureApplicantProfile = async (req: Request, res: Response) => {
    try {
        ...
        // base record (from profiles) – needed to derive email/seed
        ...
        if (baseErr) {
            console.error("ensure: profiles fetch error:", baseErr.message);
            // don’t fail the check; still proceed to see if applicant profile exists
        }
        if (!base) {
            // If you prefer, you can return 404. I’ll return 200 + exists:false with no seed.
            return res.status(200).json({ exists: false });
        }

        // check applicant_profiles by id (id === profiles.user_id)
        const { data: ap, error: apErr } = await supabase
            .from("applicant_profiles")
            .select("id, email, first_name, last_name, skills, experience_years, photo_url, resume_url, address, phone, Curent_Job_Title, Professional_Bio, Portfolio_link, Linkedin_Profile")
            .eq("id", userId)
            .maybeSingle();

        if (apErr) {
            console.error("ensure: applicant_profiles fetch error:", apErr.message);
            return res.status(500).json({ message: apErr.message });
        }

        if (ap) {
            return res.status(200).json({ exists: true, profile: ap });
        }

        // Not present → provide minimal seed the UI can use (optional)
        return res.status(200).json({
            exists: false,
            seed: { id: userId, email: base.email ?? null, first_name: base.first_name ?? null, last_name: base.last_name ?? null },
        });
    } catch (err: any) {
